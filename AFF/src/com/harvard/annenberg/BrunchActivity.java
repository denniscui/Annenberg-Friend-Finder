 ...
 */
public class BrunchActivity extends ExpandableListActivity {

	private ExpandableListView brunch;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.brunch_activity);
		...
		// set up list view
		brunch = (ExpandableListView) findViewById(android.R.id.list);
		expListAdapter = new MenuAdapter(this, createGroupList(),
				new String[] { "categoryName" }, createChildList(),
				new String[] { "foodName" }, brunch);

		brunch.setAdapter(expListAdapter);
		brunch.setOnChildClickListener(brunchListener);
		brunch.setGroupIndicator(null);
	}
	...
	private OnChildClickListener brunchListener = new OnChildClickListener() {
		public boolean onChildClick(ExpandableListView parent, View v,
				int groupPosition, int childPosition, long id) {
			String recipeId = orderedMenu.get(groupPosition).get(childPosition)
					.getRecipe();

			FetchRecipe fetcher = new FetchRecipe(BrunchActivity.this, recipeId);
			fetcher.doFetch();
			return true;
		}
