 ...
 * Displays lunch menu.
 */
public class LunchActivity extends ExpandableListActivity {

	private ExpandableListView lunch;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// set up list view
		lunch = (ExpandableListView) findViewById(android.R.id.list);
		expListAdapter = new MenuAdapter(this, createGroupList(),
				new String[] { "categoryName" }, createChildList(),
				new String[] { "foodName" }, lunch);

		lunch.setAdapter(expListAdapter);
		lunch.setOnChildClickListener(lunchListener);
		lunch.setGroupIndicator(null);
	}
	...
	private OnChildClickListener lunchListener = new OnChildClickListener() {
		public boolean onChildClick(ExpandableListView parent, View v,
				int groupPosition, int childPosition, long id) {
			String recipeId = orderedMenu.get(groupPosition).get(childPosition)
					.getRecipe();

			FetchRecipe fetcher = new FetchRecipe(LunchActivity.this, recipeId);
			fetcher.doFetch();
			return true;
		}
